...
export const placeItemsIntoCells = (args: {
  ...
  while(itemsLeftToPlace > 0) {
    ...
    args.indexesOfEmptyCells = args.indexesOfEmptyCells.filter((_, index) => index !== randomPointer);
    itemsLeftToPlace--;
  }
  return newField;
};
...
export const setInitialFieldInfo = (args: {
  ...
  const fillingProbability = args.fillingPercentage > 100
    ? 1
    : args.fillingPercentage < 0
      ? 0
      : args.fillingPercentage / 100;
  const indexesOfEmptyCells = field.map(row => row.map(cell => ({x: cell.x, y: cell.y}))).flat();
...
export const updateSize = (args: {
  ...
  const fillingProbability: number = args.fillingPercentage > 100
    ? 1
    : args.fillingPercentage < 0
      ? 0
      : args.fillingPercentage / 100;
